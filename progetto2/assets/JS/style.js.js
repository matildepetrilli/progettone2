...
if(pets1.length>0){
  let primoelemento =pets1.shift();
  pets1.push(primoelemento);
}
...
const cars = [
 ...
 cars.forEach(Obj =>{Obj.licensePlate="12345;"});
...
for(let i=0;i<cars.length; i++){
  if(cars[i].justTrims && cars[i].justTrims.length>0){
    justTrims.push(cars[i].justTrims[0]);
  }
}
